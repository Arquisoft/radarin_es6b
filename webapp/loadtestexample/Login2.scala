...
class Login2 extends Simulation {
	...
	val scn = scenario("Login2")
		...
		.exec(http("Login2_1")
			...
			.headers(headers_1)
			.resources(http("Login2_2")
			.get("/favicon.ico")
			.headers(headers_2)
			.check(bodyBytes.is(RawFileBody("/login2/0002_response.dat"))))
			.check(bodyBytes.is(RawFileBody("/login2/0001_response.html"))))
		.pause(1)
		.exec(http("Login2_3")
			...
			.get("/jwks")
			.headers(headers_3)
			.check(bodyBytes.is(RawFileBody("/login2/0004_response.json"))),
            http("Login2_5")
			.post("/register")
			.headers(headers_5)
			.body(RawFileBody("/login2/0005_request.json"))
			.check(bodyBytes.is(RawFileBody("/login2/0005_response.json"))),
            http("Login2_6")
			...
			.headers(headers_6)
			.check(bodyBytes.is(RawFileBody("/login2/0006_response.html"))),
            http("Login2_7")
			.get("/common/css/bootstrap.min.css")
			.headers(headers_7)
			.check(bodyBytes.is(RawFileBody("/login2/0007_response.css"))),
            http("Login2_8")
			.get("/common/css/solid.css")
			.headers(headers_7)
			.check(bodyBytes.is(RawFileBody("/login2/0008_response.css"))))
			.check(bodyBytes.is(RawFileBody("/login2/0003_response.json"))))
		.pause(15)
		.exec(http("Login2_9")
			.post("/login/password")
			.headers(headers_9)
			...
			.check(bodyBytes.is(RawFileBody("/login2/0009_response.html"))))
